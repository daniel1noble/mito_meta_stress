...
## Loading some important packages. For first time use, unannotate and install the two packages first
  ...
  check_and_install(c("pacman"))
	pacman::p_load(tidyverse, flextable, latex2exp, metafor, orchaRd, readxl, ape, phytools,here, brms, ggrepel, patchwork, ENmisc)
...
### Thermal developmental impacts on mitochondrial function
...
#| fig-cap: "Orchard plots showing the overall meta-analytic mean effect size and 95% prediction intervals for each stressor category."
    ...
    plots  <- lapply(MLMA_list, function(x) orchard_plot(x, xlab = TeX("$SMD_{H}$"), group = "study") +  
                                                ylim(-5,5) + scale_x_discrete(labels = "Overall") +
    ...
    (plots[[1]] + ggtitle("Corticosterone") + plots[[2]] + ggtitle("Social Deprivation") + plots[[3]] + ggtitle("Disturbance")) / ( plots[[4]] + ggtitle("Nutrition") + plots[[5]] + ggtitle("Temperature")) + plot_annotation(tag_levels = "a", tag_suffix = ")") 
