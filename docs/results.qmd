...
## Loading some important packages. For first time use, unannotate and install the two packages first
  ...
  check_and_install(c("pacman"))
	pacman::p_load(tidyverse, flextable, latex2exp, metafor, orchaRd, readxl, ape, phytools,here, brms, ggrepel, patchwork)
...
flextable(i2)  %>% compose(part = "header", value = as_paragraph(as_equation(eq))) %>% 
  autofit()
```
