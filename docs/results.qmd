...
rerun = FALSE
  if(rerun){
    ...
    for(i in 1:length(data_lists)){
      ...
      if(i == 2){
          MLMR_list_RelDur_preprost[[i]] <- rma.mv(SMDH ~ scale(relative_total_duration, scale = FALSE), V = v_SMDH, 
                            random = list(~1| study, 
                                          ~1| depend,
                                          ~1| obs),
                          test = "t",  
          ...
          MLMR_list_RelDur_preprost[[i]] <- rma.mv(SMDH ~ scale(relative_total_duration, scale = FALSE) + stage, V = v_SMDH, 
                            random = list(~1| study, 
                                          ~1| species_phylo,
                                          ~1| depend,
                                          ~1| tissue_sum,
...
#| fig-cap: "Orchard plots showing the overall meta-analytic mean effect size and 95% prediction intervals for each stressor category for the different measurement types."
    ...
    plots  <- lapply(MLMR_list_taxa[c(1,3,4)], function(x) orchard_plot(x, mod = "class", xlab = TeX("$SMD_{H}$"), group = "study", trunk.size = 1.2, branch.size = 2) +  ylim(-10,10) +                                              theme_classic() + theme(legend.position = "top", axis.title = element_text(size = 18),
                                                      axis.text = element_text(size = 12),
                                                       plot.tag = element_text(size = 24)))  
                                                                        #+ #scale_color_manual(values = "black") + #scale_fill_manual(values = "gray"))
...
ann2 <- function() {list(add_phylopic(img = mammal, x = 4.2, y = -8, height = 0.45),
    ...
    ggsave(here("output", "figures", "fig3.png"), p_taxa, width = 8.6, height = 11)
...
#| fig-cap: "Orchard plots showing the overall meta-analytic mean effect size and 95% prediction intervals for each stressor category for pre-natal versus postnatal manipulations."
    ...
    plots  <- lapply(MLMR_list_RelDur_preprost[c(1,3,4)], function(x) orchard_plot(x, mod = "stage", xlab = TeX("$SMD_{H}$"), group = "study", trunk.size = 1.2, branch.size = 2) +  ylim(-10,10) +                                              theme_classic() + theme(legend.position = "top", axis.title = element_text(size = 18),
                                                      axis.text = element_text(size = 12),
                                                       plot.tag = element_text(size = 24))) 
                                                                        #+ #scale_color_manual(values = "black") + #scale_fill_manual(values = "gray"))
    ...
    ggsave(here("output", "figures", "fig5.png"), p_prepost, width = 8.6, height = 11)
