...
## Loading some important packages. For first time use, unannotated and install the two packages first
  ...
  check_and_install(c("pacman"))
	pacman::p_load(tidyverse, flextable, latex2exp, metafor, orchaRd, readxl, ape, phytools,here, brms, ggrepel, patchwork, magick, ggtree, equatags, rotl, rphylopic, ggimage, rmarkdown, knitr, kableExtra, janitor, minqa)
...
After quantifying levels of heterogeneity in each of the different stressor datasets, we fit a series of multi-level meta-regression (MLMR) models to test our key questions. In all models, we included the same random effects as we used in our MLMA models, however, MLMR models differed in the moderators included because each stressor type had unique sources of heterogeneity that we expected would explain effect variance. A number of moderators were relevant to all stressor manipulation types and were included in all models. These included: 1) measurement trait category (i.e., antioxidants, oxidative damage etc.), 2) relative total duration of stressor manipulation and stage of manipulation (pre-natal/post-natal or both), and 3) taxa (i.e., class: mammals, birds, amphibians, fish). Some stressor categories were made up of a single class (e.g., social deprivation was only done in mammals) and so we did not include the taxa moderator in these models. We included relative total duration of stressor manipulation as a continuous moderator because we expected that longer durations of stressor manipulations would have larger impacts on mitochondrial function. We fit models assuming heteroscedastic variation among levels of a given moderators to capture differences in effect variation. 
...
```{r, RelDur_preprost}
#| label: RelDur_preprost
#| echo: false
#| warning: false
#| error: false

#### -------------------------------------------------- ####
# 1.3   Relative total duration of development stressor and stage 
#### -------------------------------------------------- ####
...
rerun = FALSE
  if(rerun){
    ...
    # Create list to store models
    MLMR_list_RelDur_preprost <- list()
    
    for(i in 1:length(data_lists)){
      # Add observation level random effect
        data_lists[[i]]$obs <- 1:nrow(data_lists[[i]])
      # Scale the relative total duration of stressor to development time  
        data_lists[[i]]$sc_rel_total <- scale(data_lists[[i]]$relative_total_duration, scale = FALSE)
      ...
      if(i == 2){
          MLMR_list_RelDur_preprost[[i]] <- rma.mv(SMDH ~ sc_rel_total, V = v_SMDH, 
                            random = list(~1| study, 
                                          ~1| obs),
                          test = "t",  
                          data = data_lists[[i]])
      } else {
          MLMR_list_RelDur_preprost[[i]] <- rma.mv(SMDH ~ sc_rel_total + stage, V = v_SMDH, 
                            random = list(~1| study, 
                                          ~1| species_phylo,
                                          ~1| tissue_sum,
                                          ~1 + stage | obs),
      ...
      # Add names to make sure we keep track of which dataset is which model                      
        names(MLMR_list_RelDur_preprost)[i] <- names(data_lists)[i]
    }
      
    # Save the model
        saveRDS(MLMR_list_RelDur_preprost, file = here("output", "models", "MLMR_list_RelDur_preprost.rds"))

  } else {
        MLMR_list_RelDur_preprost <- readRDS(here("output", "models", "MLMR_list_RelDur_preprost.rds"))
  }


  # RVE to check robustness of results
  robust_RelDur <- lapply(MLMR_list_RelDur_preprost, function(x) robust(x, cluster = x$data[,"study"]))
  #hist(residuals(robust_RelDur$cort_stress.csv))
  #plot(data_lists[[1]]$SMDH, data_lists[[1]]$relative_total_duration) 
```
...
In total, we collected data from `r sum(i2$k)` effect sizes across `r length(unique(data$species_phylo))` species (@fig-phylo) and `r length(unique(data$study))` studies across the different stressors (@tbl-heterogeneity). 

## Corticosterone developmental impacts on mitochondrial function

Overall, exposure to corticosterone during development negatively impacted mitochondrial function ($SMD_{H}$ = `r coef(summary_list[[1]])$estimate`, 95% CI = `r coef(summary_list[[1]])$ci.lb` to `r coef(summary_list[[1]])$ci.ub`, $p$ = `r coef(summary_list[[1]])$pval`; [@fig-orchard_int]a). However, effect heterogeneity was high (95% Prediction Intervals: `r predict(summary_list[[1]])$pi.lb` to `r predict(summary_list[[1]])$pi.ub`; @tbl-heterogeneity), with `r i2[1,"I2_study"]`% of variation being driven by differences among studies ($I^2_{study}$; @tbl-heterogeneity) and `r i2[1,"I2_tissue_sum"]`% of variation being driven by differences among tissues ($I^2_{tissue}$; @tbl-heterogeneity) with little to no variation explained by phylogeny (@tbl-heterogeneity). 

## Social deprivation developmental impacts on mitochondrial function
Social deprevation studies were only ever done with mammals with all studies manipulating environments postnatally. Social deprivation during development had a negative impact on mitochondrial function ($SMD_{H}$ = `r coef(summary_list[[2]])$estimate`, 95% CI = `r coef(summary_list[[2]])$ci.lb` to `r coef(summary_list[[2]])$ci.ub`, $p$ = `r coef(summary_list[[2]])$pval`; [@fig-orchard_int]b). Effect heterogeneity was again high (95% Prediction Intervals: `r predict(summary_list[[2]])$pi.lb` to `r predict(summary_list[[2]])$pi.ub`; @tbl-heterogeneity), with `r i2[4,"I2_study"]`% of the variation being driven by differences among studies ($I^2_{study}$) (@tbl-heterogeneity). 

## Disturbance developmental impacts on mitochondrial function

Disturbance during development had a negative impact on mitochondrial function ($SMD_{H}$ = `r coef(summary_list[[3]])$estimate`, 95% CI = `r coef(summary_list[[3]])$ci.lb` to `r coef(summary_list[[3]])$ci.ub`, $p$ = `r coef(summary_list[[3]])$pval`; [@fig-orchard_int]c). Effect heterogeneity was high (95% Prediction Intervals: `r predict(summary_list[[3]])$pi.lb` to `r predict(summary_list[[3]])$pi.ub`; @tbl-heterogeneity), with substantial variation across studies ($I^2_{study}$ = `r i2[2,"I2_study"]`%)(@tbl-heterogeneity).

Disturbance stress occured only postnatally in fish and birds, whereas in mammals disturbance stressors were also applied prenatally. 

## Nutrition developmental impacts on mitochondrial function
Nutritional stress during development had limited impact on mitochondrial function overall ($SMD_{H}$ = `r coef(summary_list[[4]])$estimate`, 95% CI = `r coef(summary_list[[4]])$ci.lb` to `r coef(summary_list[[4]])$ci.ub`, $p$ = `r coef(summary_list[[4]])$pval`; [@fig-orchard_int]d). Effect heterogeneity was high (95% Prediction Intervals: `r predict(summary_list[[4]])$pi.lb` to `r predict(summary_list[[4]])$pi.ub`; @tbl-heterogeneity), with variation being driven by differences between studies ($I^2_{study}$ = `r i2[3,"I2_study"]`%),  phylogeny ($I^2_{phylogeny}$ = `r i2[3,"I2_species_phylo"]`%) and tissue ($I^2_{tissue}$ = `r i2[3,"I2_tissue_sum"]`%) (@tbl-heterogeneity).
...
# Have a look at categories and levels to identify gaps 
  tabyl(nutri_data, nutrition_type, nutrition_sum) 
  tabyl(nutri_data, nutrition_type, nutrition_sum, measurement_category) 
  tabyl(nutri_data, nutrition_type, measurement_category) 
  tabyl(nutri_data, nutrition_sum, measurement_category) 
  tabyl(nutri_data, nutrition_type, class) 
  tabyl(nutri_data, nutrition_sum, class) 
...
rerun = FALSE
  ...
  # Have a look at the model with orchaRd
     orchard_plot(nutri_up_down, mod = "nutrition_type", xlab = TeX("$SMD_{H}$"), group = "study", trunk.size = 1.2, branch.size = 2) +  
      ylim(-10,10) +
      theme_classic() 
  ...
  # RVE to check robustness of results
 # robust_taxa <- lapply(nutri_up_down[c(1,3,4)], function(x) robust(x, cluster = x$data[,"study"]))
...
z = list(data.frame(x = c(1.2, 2.2, 3.2, 4.2), y = rep(5, 4)),
         ...
         data.frame(x = c(1.2, 2.2, 3.2), y = rep(5, 3)),
         data.frame(x = c(1.2, 2.2, 3.2, 4.2, 5.2), y = rep(6,5)))
  
# Apply orchard to each model and add the k and spp information to the plot
  plots  <- mapply(function(x, y, z) orchard_plot(x, mod = "measurement_category", xlab = TeX("$SMD_{H}$"), group = "study", trunk.size = 1.2, branch.size = 2, k = FALSE, g = FALSE) +  ylim(-10,10) +                                              theme_classic() + theme(legend.position = "top", axis.title = element_text(size = 18),
                                                      axis.text = element_text(size = 12),
...
z = list(data.frame(x = c(1.2, 2.2, 3.2), y = rep(5, 3)),
    ...
    plots  <- mapply(function(x, y, z)  {orchard_plot(x, mod = "class", xlab = TeX("$SMD_{H}$"), group = "study", trunk.size = 1.2, branch.size = 2, g = FALSE, k = FALSE) +  ylim(-10,10) +                                              theme_classic() + theme(legend.position = "top", axis.title = element_text(size = 18),
                                                      axis.text = element_text(size = 12),
                                                       plot.tag = element_text(size = 24))+
...
z = list(data.frame(x = c(1.2, 2.2), y = rep(5, 2)),
    ...
    plots  <- mapply(function(x,y,z){ orchard_plot(x, mod = "stage", xlab = TeX("$SMD_{H}$"), group = "study", trunk.size = 1.2, branch.size = 2, k = FALSE, g = FALSE, at = list( sc_rel_total= c(-0.5, 0, 0.5)), by = "sc_rel_total", weights = "prop") +  ylim(-10,10) +                                              theme_classic() + theme(legend.position = "top", axis.title = element_text(size = 18),
                                                      axis.text = element_text(size = 12),
                                                       plot.tag = element_text(size = 24)) +
    annotate("text", x = c(z$x), y = c(z$y), label = TeX(paste0("\\textit{k} = ", y$k, " (", y$studies, ")")), size = 5, hjust = 0)}, x = MLMR_list_RelDur_preprost[c(1,3,4)], y = prepost_table[c(1,3,4)], z = z, SIMPLIFY = FALSE)
...
```{r, tbl-heterogeneity_cv_m2}
#| label: tbl-heterogeneity_cv_m2
#| tbl-cap: "Heterogeneity estimates for developmental stressor datasets"
#| echo: false
#| warning: false
#| error: false
...
#### -------------------------------------------------- ####
#        Heterogeneity M2 and CV2 Tables
#### -------------------------------------------------- ####
...
cv2  <- cv2  %>% select(Stressor, everything())
m2$Stressor <- c("Corticosterone",  "Disturbance", "Nutrition", "Social Deprevation")
m2  <- m2  %>% select(Stressor, everything())

# bind together
cv2 <- bind_cols(cv2, m2[, -1]) 
...
flextable(round_df(cv2, digits = 3))  %>% compose(part = "header", value = as_paragraph(as_equation(eq))) %>% 
  width(j = c(1:9), width = c(1.5, rep(0.65, 8)))  %>%  compose(j = c(7,8), i = 4, value = as_paragraph("–"))
```
...
#### -------------------------------------------------- ####
#  Checking how data is distributed across levels in models
#### -------------------------------------------------- ####

  # Create a table of the number of effects per level
  breakdowns <- lapply(data_lists, function(x) tabyl(x, measurement_category, stage, class))
