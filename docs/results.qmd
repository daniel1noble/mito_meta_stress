...
We fit models assuming heteroscedastic variation among levels of moderators to capture differences in effect variation. 
...
#### -------------------------------------------------- ####
  ...
  i2  <- lapply(MLMA_list[c(1,3,4)], function (x) orchaRd::i2_ml(x))
  i2  <- data.frame(do.call("rbind", i2))
  i2.2  <- lapply(MLMA_list[2], function (x) orchaRd::i2_ml(x))
  i2.2 <- data.frame(do.call("rbind", i2.2))
  i2[4,c(1,2,5)] <- i2.2
  i2  <- cbind(size, i2)
...
#| fig-cap: "Orchard plots showing the overall meta-analytic mean effect size and 95% prediction intervals for each stressor category for the different measurement types."
    ...
    plots  <- lapply(MLMR_list_Measure, function(x) orchard_plot(x, mod = "measurement_category", xlab = TeX("$SMD_{H}$"), group = "study", trunk.size = 1.2, branch.size = 2, k = FALSE, g = FALSE) +  ylim(-10,10) +                                              theme_classic() + theme(legend.position = "top", axis.title = element_text(size = 18),
                                                      axis.text = element_text(size = 12),
...
#| fig-cap: "Orchard plots showing the overall meta-analytic mean effect size and 95% prediction intervals for each stressor category for the different measurement types."
    ...
    plots  <- lapply(MLMR_list_taxa[c(1,3,4)], function(x) orchard_plot(x, mod = "class", xlab = TeX("$SMD_{H}$"), group = "study", trunk.size = 1.2, branch.size = 2, g = FALSE, k = FALSE) +  ylim(-10,10) +                                              theme_classic() + theme(legend.position = "top", axis.title = element_text(size = 18),
                                                      axis.text = element_text(size = 12),
                                                       plot.tag = element_text(size = 24)))  
                                                                        #+ #scale_color_manual(values = "black") + #scale_fill_manual(values = "gray"))
...
#| fig-cap: "Orchard plots showing the overall meta-analytic mean effect size and 95% prediction intervals for each stressor category for pre-natal versus postnatal manipulations."
    ...
    plots  <- lapply(MLMR_list_RelDur_preprost[c(1,3,4)], function(x) orchard_plot(x, mod = "stage", xlab = TeX("$SMD_{H}$"), group = "study", trunk.size = 1.2, branch.size = 2, k = FALSE, g = FALSE, at = list( sc_rel_total= c(-0.5, 0, 0.5)), by = "sc_rel_total", weights = "prop" ) +  ylim(-10,10) +                                              theme_classic() + theme(legend.position = "top", axis.title = element_text(size = 18),
                                                      axis.text = element_text(size = 12),
                                                       plot.tag = element_text(size = 24))) 
                                                                        #+ #scale_color_manual(values = "black") + #scale_fill_manual(values = "gray"))
